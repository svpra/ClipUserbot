...
try:
    ...
    async def Progressbar(client: Client, message: Message):
        try:
            ...
            for i in range(total + 1):
                ...
                time.sleep(0.0001)
                await message.edit(
                    text + "\n[{:{}}] {:>3}%".format("█" * int(percent / (100.0 / bar_length)), bar_length,
                                                     int(percent)))
        except Exception as erryr:
            now = datetime.datetime.now()
            timnow = now.strftime("Дата %d.%m.%Y • Время %H:%M:%S")
    ...
    async def myt(client: Client, message: Message):
        try:
            ...
            await message.edit(
                "⏳ | Скачивание аудиодорожки. Это займёт некоторое время (зависит от размера аудиодорожки)")
            os.system(myth)
            await message.edit(
                "⏳ | Отправка аудиодорожки. Это займёт некоторое время (зависит от размера аудиодорожки)")
    ...
    async def mute_hammer(client: Client, message: Message):
        ...
        if await CheckAdmin(message) is True:
            ...
            if reply:
                ...
                if not user:
                    await message.edit(
                        "👀 | Не вижу пользователя, которого требуется **ограничить в отправке сообщений.**")
                    return
            try:
                get_user = await app.get_users(user)
    ...
    async def unmute(client: Client, message: Message):
        ...
        if await CheckAdmin(message) is True:
            ...
            try:
                ...
                await message.edit(
                    f"✅ | Пользователь {get_user.first_name} больше **не ограничен в отправке сообщений.**")
            except:
